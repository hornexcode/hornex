    ...
    >
      {PriceOptions.map((item, index) => (
        <RadioGroup.Option value={item.value} key={index}>
          {({ checked }) => (
            <span
              className={`shadow-card hover:shadow-large dark:bg-medium-dark relative flex cursor-pointer items-center justify-center rounded  border-2 border-transparent bg-white text-center text-sm font-medium transition-all ${
                checked
                  ? 'border-light-dark dark:border-gray-500'
                  : 'border-gray-400'
              }`}
            >
